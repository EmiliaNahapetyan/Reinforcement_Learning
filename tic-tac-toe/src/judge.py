...
class Judge:
    ...
    def play(self, all_states, print_state: bool = False):
        ...
        while True:
            ...
            # Make an action - handle both list and tuple return types
            action = player.act(3, 3)

            # Handle different return types from player.act()
            if isinstance(action, (list, tuple)):
                if len(action) == 3:
                    i, j, symbol = action
                else:
                    # Handle case where action is a list like [i, j] with symbol appended
                    i, j = action[:2]
                    symbol = player.symbol
            else:
                # Unexpected return type
                raise TypeError("Player.act() returned an unexpected type")
