...
def play():
    ...
    # Check if policy files exist, train if needed
    if not (os.path.exists('policy_first.bin') and os.path.exists('policy_second.bin')):
        print("Training RL player first... This might take a while.")
        train(epochs=int(1e4))  # Reduced number of epochs for faster training
        print("Training complete! Now you can play against the RL player.")
